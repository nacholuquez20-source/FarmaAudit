...
class SheetsManager:
    ...
    def _initialize(self) -> None:
        ...
        try:
            service_account_json = self._load_service_account_json(settings.google_service_account_json)
    ...
    @staticmethod
    def _load_service_account_json(env_value: Optional[str]) -> str:
        """Accept base64, raw JSON, or fall back to local service-account.json."""
        candidates: List[str] = []
        raw = (env_value or "").strip()
        if raw:
            candidates.append(raw)

        fallback_path = Path("service-account.json")
        if fallback_path.exists():
            candidates.append(fallback_path.read_text(encoding="utf-8").strip())
        ...
        raise ValueError("Unable to load Google service account JSON from env or service-account.json")
